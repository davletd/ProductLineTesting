...
// Large requires ArmySpecialUnit
fact Invariant_1
{
	all c:Configuration | #c.f63==1 implies (#c.f39=1)
}   

// ElecLine requires Electricity
fact Invariant_2
{
	all c:Configuration | #c.f17==1 implies (#c.f57=1)
}   

// TelecomLine requires Telecom
fact Invariant_3
{
	all c:Configuration | #c.f18==1 implies (#c.f58=1)
}   

// Constraints due to the Operators
// CrisisManagementSystem And Operator =>  LongDistanceCall AND  Coordinator AND  Mission AND  ServiceUsed AND  Area AND  CarCrash AND   
